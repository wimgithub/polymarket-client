...
func (c *Client) BuildSplitPositionTx(req *SplitPositionRequest, out *CTFTransaction) error {
	...
	if err := validateBigIntSliceRequired("partition", req.Partition); err != nil {
		return err
	}
...
func validateHashRequired(name string, h common.Hash) error {
	if h == (common.Hash{}) {
		return fmt.Errorf("polymarket: %s is required", name)
	}
	return nil
...
func validateBigIntRequired(name string, v *big.Int) error {
	if v == nil {
		return fmt.Errorf("polymarket: %s is required", name)
	}
	if v.Sign() <= 0 {
...
func validateBigIntSliceRequired(name string, values []*big.Int) error {
	if len(values) == 0 {
		return fmt.Errorf("polymarket: %s is required", name)
	}
	for i, v := range values {
		if v == nil {
			return fmt.Errorf("polymarket: %s[%d] is required", name, i)
		}
		if v.Sign() <= 0 {
			return fmt.Errorf("polymarket: %s[%d] must be positive", name, i)
		}
	}
	return nil
}
